...
var response;
Future<Map> getPexelsLatestImages(int perPage, int pageNumer, _) async {
  if (response == null) {
    response = await http.get(Uri.encodeFull(
        "https://pixabay.com/api/?key=14951209-61b2f6019e4d1a85e007275aa" +
            "&order=latest&page=$pageNumer&per_page=$perPage&editors_choice=true&category=$_"));
  }
...
Future<Map> getPexelsPopularImages(int perPage, int pageNumer, _) async {
  if (response == null) {
    response = await http.get(Uri.encodeFull(
        "https://pixabay.com/api/?key=14951209-61b2f6019e4d1a85e007275aa" +
            "&order=popular&page=$pageNumer&per_page=$perPage&editors_choice=true&category=$_"));
  }
  if (response.statusCode == 200) {
    var image = json.decode(response.body);
    imageModel = new ImageModel.fromJson(image);
